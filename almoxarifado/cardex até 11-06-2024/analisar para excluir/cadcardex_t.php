...
$qtde_valid = str_replace(',','.', filter_var($_POST['qtde']));
$saldo_item_valid = filter_var($_POST['saldo_item']);
//$saldo_item_valid = (decimal(18,5))filter_var($_POST['saldo_item']);
$cod_item_valid = filter_var($_POST['cod_item']);
$soma = $saldo_item_valid - $qtde_valid;

if (filter_var($movto_valid) == 'S' && $soma < 0){	
	echo "Sado após Lançamento ".$soma;
	echo "<br>Movimento: ".$movto_valid;
	echo "<br>Quantidade: ".$qtde_valid;
	echo "<br>Sado Item: ".$saldo_item_valid;
	echo "<br>Cód. Produto: ".$cod_item_valid;
